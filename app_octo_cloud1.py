...
# Prompt template for club recommendations
club_recommendation_prompt = PromptTemplate(
    input_variables=["user_interests", "recommended_clubs"],
    template="""
...
# LLM chain for club recommendations
club_recommendation_chain = LLMChain(
    llm=chat_model,
    prompt=club_recommendation_prompt
)

# For general conversation (non-recommendation) we create a simple prompt chain
...
def extract_interests(user_input):
    ...
    This is a simple implementation - in a real app, you might use NER or other techniques
    """
    # List of common interests that might be mentioned
    common_interests = [
...
if prompt := st.chat_input("Tell me about your interests..."):
    ...
    if is_interest_query(prompt) or is_followup_query(prompt) or st.session_state.detected_interests:
        ...
        if clubs:
            ...
            # Generate recommendations using the LLM chain
            response_data = club_recommendation_chain.invoke({
                "user_interests": f"{prompt} (Detected interests: {', '.join(st.session_state.detected_interests)})",
                "recommended_clubs": clubs_text
            })
            response_text = response_data.get('text', "Sorry, I couldn't generate recommendations.")
        else:
        ...
        response_text = response_data.get('text', "Sorry, I didn't understand that. Could you tell me more about your interests so I can recommend some clubs?")
